...
class TestStateTransitions:
    ...
    def test_acwr_dict_populated_after_create(self, mock_agents, runtime_db, runtime_svc):
        ...
        assert isinstance(acwr["ratio"], float)
        assert acwr["status"] in {"safe", "caution", "danger"}
    ...
    def test_budgets_populated(self, mock_agents, runtime_db, runtime_svc):
        ...
        assert all(isinstance(v, (int, float)) for v in budgets.values())
        assert "running" in budgets  # SIMON_PROFILE has running + lifting
    ...
    def test_recommendations_populated(self, mock_agents, runtime_db, runtime_svc):
        ...
        assert all("agent_name" in r for r in recs)
