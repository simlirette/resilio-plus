...
def _db_mock(sessions_completed: int = 3):
    ...
    # Mock SessionLogModel query: returns a count for filter().count()
...
def test_graph_no_interrupt_human_approved_false_does_not_write_db():
    ...
    # DB commit should NOT be called (no review persisted)
...
def test_analyze_actual_vs_planned_no_plan():
    ...
    assert partial["completion_rate"] == 0.0  # 0 planned → 0.0
...
def test_analyze_actual_vs_planned_computes_completion_rate():
    ...
    # Build a mock plan with sessions for this week
    mock_plan = MagicMock()
...
def test_update_athlete_state_builds_summary():
    ...
    state = {
        **state,
        ...
        "actual_hours": 3.5,
        "acwr_dict": {"ratio": 1.1, "status": "safe", "acute_7d": 3.5, "chronic_28d": 3.2, "max_safe_weekly_load": 5.0},
    }
    config = {"configurable": {"thread_id": "t1", "db": MagicMock()}}
...
def test_graph_with_interrupt_pauses_before_present_review():
    ...
    # After interrupt, db_review_id should still be None (not yet applied)
    assert result.get("db_review_id") is None
    # review_summary_dict should be populated (nodes 1-3 ran)
...
def test_graph_interrupt_then_resume_declined():
    ...
    # Phase 2: inject rejection and resume
