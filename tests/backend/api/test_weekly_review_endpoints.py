...
def test_review_confirm_approved(client_with_athlete):
    ...
    mock_svc.resume_review.assert_called_once_with(
        thread_id=THREAD_ID,
        approved=True,
        db=mock_svc.resume_review.call_args.kwargs["db"],
    )


def test_review_confirm_declined(client_with_athlete):
    """POST /plan/review/confirm with approved=False calls resume_review and returns annulée."""
    client = client_with_athlete
...
# ---------------------------------------------------------------------------
# Authorization: wrong athlete cannot start/confirm review
# ---------------------------------------------------------------------------
