...
// SVG chevron arrows — perfectly centered, no baseline issues
const chevronL = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 3L5 7L9 11" stroke="#52525b" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
const chevronR = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M5 3L9 7L5 11" stroke="#52525b" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* FIX 3: Arrow buttons — SVG instead of text characters, proper centering */
  .nav-arrow {
  ...
  /* ── CALENDAR LAYOUT ──
     .cal-outer (flex row)
       .time-col (44px, fixed)
       .cal-body (flex:1, flex col) ← headers + scrollable grid
       .detail-panel (0 → 264px)
  */
  ...
  .time-col-scroll { flex:1; overflow:hidden; position:relative; }
  ...
  /* Day headers — always spans exact same width as .day-cols */
  .day-headers { display:flex; flex-shrink:0; border-bottom:1px solid #e4e4e7; height:40px; }
  .day-hcell { flex:1; min-width:0; padding:5px 8px 4px; border-right:1px solid #e4e4e7; display:flex; flex-direction:column; justify-content:center; }
  ...
  /* FIX 2: Hide scrollbar so it doesn't steal width from day-cols.
     Day-headers and day-cols will now have identical widths. */
  .day-scroll {
    flex:1;
  ...
  .day-scroll::-webkit-scrollbar { display:none; } /* Chrome/Safari: hide */
  ...
  .day-col {
    ...
    background-image: repeating-linear-gradient(
      to bottom,
      transparent 0px, transparent ${PX_PER_HOUR - 1}px,
      #f0f0f0 ${PX_PER_HOUR - 1}px, #f0f0f0 ${PX_PER_HOUR}px
    );
  ...
  /* FIX 1: Session block — column layout, bigger icon, time below */
  .sblock {
    position:absolute;
    left:4px; right:4px;
    border-radius:7px;
  ...
  .sport-dot img { width:16px; height:16px; object-fit:contain; filter:invert(1); }

  /* FIX 1: Duration below icon */
  .sblock-dur {
    font-size:10px;
  ...
  .detail-close { background:transparent; border:none; padding:2px 4px; cursor:pointer; color:#a1a1aa; font-size:15px; line-height:1; display:flex; align-items:center; justify-content:center; border-radius:4px; }
  ...
  <div class="main">
    ...
    <div class="cal-outer" id="calOuter">
      ...
      <div class="time-col">
        ...
        <div class="time-col-scroll" id="timeColScroll">
          <div class="time-gutter">${timeLabels}</div>
        </div>
      </div>

      <!-- Cal body: headers + grid (no scrollbar width issues) -->
      <div class="cal-body">
        ...
        <div class="day-scroll" id="dayScroll">
          <div class="day-cols">
            ...
            <div class="day-col">
              <div class="sblock done" style="top:${top(7,0)}px;height:${height(45)}px" onclick="openDetail()">
            ...
            <!-- MAR: 1h 15 vélo done -->
            <div class="day-col">
              <div class="sblock done" style="top:${top(7,0)}px;height:${height(75)}px" onclick="openDetail()">
            ...
            <div class="day-col today-col">
              ...
              <div class="sblock selected" style="top:${top(17,0)}px;height:${height(55)}px" onclick="openDetail()">
                <div class="sport-dot"><img src="${runner}" alt=""></div>
            ...
            <!-- VEN: 45 min natation -->
            <div class="day-col">
              <div class="sblock planned" style="top:${top(7,0)}px;height:${height(45)}px" onclick="openDetail()">
            ...
            <!-- SAM: 1h 30 course -->
            <div class="day-col">
              <div class="sblock planned" style="top:${top(9,0)}px;height:${height(90)}px" onclick="openDetail()">
                <div class="sport-dot"><img src="${runner}" alt=""></div>
...
<div style="text-align:center;margin-top:18px;font-size:11px;color:#71717a;font-family:'DM Sans',system-ui">
  Page 2/12 — /plan · Cliquer sur un bloc pour ouvrir le détail · ✕ pour fermer
</div>

<script>
  const calOuter    = document.getElementById('calOuter');
  const dayScroll   = document.getElementById('dayScroll');
  const timeColScroll = document.getElementById('timeColScroll');

  dayScroll.addEventListener('scroll', () => {
    timeColScroll.scrollTop = dayScroll.scrollTop;
  });
  ...
  openDetail();
  dayScroll.scrollTop = Math.max(0, ${top(14, 30)} - 180);
</script>
</body>
</html>`;

fs.writeFileSync('C:/Users/simon/resilio-plus/.superpowers/brainstorm/1005-1775853957/content/plan-v6.html', html);
console.log('done');
