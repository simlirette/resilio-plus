...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ─────────────────────────────────────────────────────────────
     CALENDAR LAYOUT
     Structure:
       .cal-outer (flex row, flex:1)
         .time-col       — 44px fixed column (gutter + header spacer)
         .cal-body       — flex:1, flex column (headers + scrollable grid)
         .detail-panel   — hidden by default, slides in on open
  ──────────────────────────────────────────────────────────── */
  .cal-outer {
    flex:1;
    display:flex;
  ...
  /* ── TIME COLUMN (fixed left, not part of scrolling body) ── */
  .time-col {
    width:44px;
    min-width:44px;
    flex-shrink:0;
  ...
  .time-col-header {
    /* same height as .day-headers row — matched by JS or fixed value */
    height:40px;
    flex-shrink:0;
  ...
  /* ── CALENDAR BODY (flex:1, scrollable vertically) ── */
  .cal-body {
    flex:1;
    display:flex;
    flex-direction:column;
  ...
  /* Day headers — always full width of cal-body */
  .day-headers {
    display:flex;
    flex-shrink:0;
    border-bottom:1px solid #e4e4e7;
  ...
  /* 7 day columns as flex row, height = full grid */
  .day-cols {
    display:flex;
    height:${GRID_HEIGHT}px;
  }
  ...
  /* ── DETAIL PANEL ── hidden by default, visible when .detail-open ── */
  .detail-panel {
    width:0;
    min-width:0;
    background:#fff;
  ...
  .detail-close {
    ...
    cursor:pointer; color:#a1a1aa; font-size:15px; line-height:1;
...
<div class="frame">
  ...
  <div class="main">
    ...
    <!-- Calendar: time-col | cal-body (headers + grid) | detail-panel -->
    <div class="cal-outer" id="calOuter">
      ...
      <!-- Calendar body: day headers + scrollable grid -->
      <div class="cal-body">
        <!-- Day headers — always flex:1 of cal-body, no detail spacer needed -->
        ...
        <div class="day-scroll" id="dayScroll">
          <div class="day-cols">
            ...
            <!-- JEU: Tempo 17h00 55min — selected -->
      ...
      <div class="detail-panel" id="detailPanel">
        <div class="detail-hd">
          ...
          <button class="detail-close" onclick="closeDetail()" title="Fermer">✕</button>
        </div>
...
<div style="text-align:center;margin-top:18px;font-size:11px;color:#71717a;font-family:'DM Sans',system-ui">
  Page 2/12 — /plan · 1280×800px &nbsp;·&nbsp; Cliquer sur un entraînement pour voir le détail
</div>
...
<script>
  ...
  /* Sync vertical scroll between grid and time gutter */
  dayScroll.addEventListener('scroll', () => {
    timeColScroll.scrollTop = dayScroll.scrollTop;
  });

  function openDetail() {
    calOuter.classList.add('detail-open');
  }
  function closeDetail() {
    calOuter.classList.remove('detail-open');
  }

  /* Auto-open detail on load so user can see the panel */
  openDetail();

  /* Scroll grid to show the now-line area (14h30 → around half the visible area) */
  const nowTop = ${top(14, 30)};
  dayScroll.scrollTop = Math.max(0, nowTop - 180);
</script>
</body>
</html>`;

fs.writeFileSync('C:/Users/simon/resilio-plus/.superpowers/brainstorm/1005-1775853957/content/plan-v5.html', html);
console.log('done');
