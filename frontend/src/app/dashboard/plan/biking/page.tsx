...
const SESSION_TYPE_BADGE: Record<string, string> = {
  endurance: "bg-emerald-900 text-emerald-200",
  tempo: "bg-amber-900 text-amber-200",
  vo2max: "bg-red-900 text-red-200",
};
...
const PHASE_LABELS: Record<string, string> = {
  ...
  main_set: "Bloc principal",
  cool_down: "Retour au calme",
};

export default function BikingPlanPage() {
  const [plan, setPlan] = useState<BikePlan | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    api
      .post<BikePlan>("/plan/biking", { athlete_state: SIMON_ATHLETE_STATE })
      .then(setPlan)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-100">Plan vélo — Détail</h2>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 text-center">
          ...
          <p className="text-2xl font-bold text-slate-100">
            {plan.weekly_volume_km != null ? `${plan.weekly_volume_km} km` : "—"}
          </p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 text-center">
          <p className="text-xs text-slate-500">FTP</p>
          <p className="text-2xl font-bold text-slate-100">
            {plan.ftp_watts != null ? `${plan.ftp_watts} W` : "N/A"}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {sessions.map((s, i) => {
          ...
          const sessionNotes = s.coaching_notes_session ?? [];
            ...
            <div key={i} className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-400">
                  ...
                  {s.tss_estimated != null && <span>TSS {s.tss_estimated}</span>}
                </div>
              </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-slate-800">
                        <th className="text-left py-1.5 pr-3 text-slate-500 font-medium">Phase</th>
                        <th className="text-left py-1.5 pr-3 text-slate-500 font-medium">Durée</th>
                        <th className="text-left py-1.5 pr-3 text-slate-500 font-medium">Puissance cible</th>
                        <th className="text-left py-1.5 text-slate-500 font-medium">Description</th>
                      </tr>
                    </thead>
                    <tbody>
                      {blocks.map((block, bi) => (
                        <tr key={bi} className="border-b border-slate-800/50 last:border-0">
                          <td className="py-1.5 pr-3 text-slate-300 whitespace-nowrap">
                            {PHASE_LABELS[block.phase] ?? block.phase}
                          </td>
                          <td className="py-1.5 pr-3 text-slate-400 whitespace-nowrap">{block.duration_minutes} min</td>
                          <td className="py-1.5 pr-3 text-slate-400 whitespace-nowrap">{block.power_target}</td>
                          <td className="py-1.5 text-slate-400 leading-relaxed">{block.description}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {sessionNotes.length > 0 && (
                <div className="space-y-1">
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Conseils</p>
                  <ul className="space-y-0.5">
                    {sessionNotes.map((note, ni) => (
                      <li key={ni} className="text-xs text-slate-400">• {note}</li>
                    ))}
                  </ul>
                </div>
              )}
