...
### Couleur
- Gradient d'accent sur cards ou backgrounds
- Couleurs sémantiques (green/yellow/red) sur UI non-physiologique (navigation, cards, labels)
- Fonds photo paysage ou photo humain
- Pastels
- Violet `#5b5fef` ou bleu `#3B74C9` — ces couleurs n'apparaissent pas dans les screenshots
- Hex codés en dur dans les composants — tout via `@resilio/design-tokens`
- Dark bg clinique pur `#08080e` — utiliser les warm charcoals

### Typographie
- Serif display ou italic dramatique
- `@import url()` pour les fonts — expo-font uniquement

### Layout
- Drop-shadows lourds (box-shadow > 8px blur)
- Pill-shape systématique sur tous les boutons (radius 50% sur CTA pleine largeur)
- Animations cosmétiques sans purpose fonctionnel (shimmer perpétuel, looping decoratif)

### Logique
- Pas de `localStorage` / `sessionStorage` (hors sujet en RN — mentionne seulement si contexte web-view)
...
- **Animations**: `react-native-reanimated` v3 exclusivement — pas `Animated` legacy, pas `LayoutAnimation`
