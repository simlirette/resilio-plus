...
`weekly_review(athlete_id, db) -> str`
...
### 2. `compute_new_acwr`
- Queries last 4 `WeeklyReviewModel` rows for `athlete_id`
- Calls `core.acwr.compute_acwr(load_history)` — falls back to simplified ratio if history empty
...
- Calls same `_build_recommendations` logic as `weekly_sync`
