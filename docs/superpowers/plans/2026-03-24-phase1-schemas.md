...
**Architecture:** Pydantic schemas live in `backend/app/schemas/` for in-memory agent/API validation. SQLAlchemy models live in `backend/app/db/` for persistence. The two layers are completely separate — no SQLModel coupling. Complex fields (lists, nested objects) are stored as JSON TEXT in SQLite. FatigueScore has no DB table — it is embedded as JSON inside WorkoutSlot.
...
| `backend/app/__init__.py` | Create | Backend `resilio` package root |
| `backend/app/schemas/__init__.py` | Create | Schemas sub-package |
| `backend/app/schemas/fatigue.py` | Create | `FatigueScore` model |
| `backend/app/schemas/athlete.py` | Create | `Sport` enum, `DayType` enum, `AthleteProfile` model |
| `backend/app/schemas/plan.py` | Create | `WorkoutSlot` model, `TrainingPlan` model |
| `backend/app/schemas/nutrition.py` | Create | `MacroTarget` model, `DayNutrition` model, `NutritionPlan` model |
| `backend/app/schemas/review.py` | Create | `ActivityResult` model, `WeeklyReview` model |
| `backend/app/db/__init__.py` | Create | DB sub-package |
| `backend/app/db/database.py` | Create | SQLAlchemy engine, `SessionLocal`, `Base`, FK pragma event hook |
| `backend/app/db/models.py` | Create | 4 ORM tables: `AthleteModel`, `TrainingPlanModel`, `NutritionPlanModel`, `WeeklyReviewModel` |
...
**Files:**
- Modify: `pyproject.toml`
- Create: `backend/__init__.py`, `backend/app/__init__.py`, `backend/app/schemas/__init__.py`, `backend/app/db/__init__.py`
...
```bash
git add pyproject.toml backend/__init__.py backend/app/__init__.py backend/app/schemas/__init__.py backend/app/db/__init__.py tests/backend/__init__.py tests/backend/schemas/__init__.py tests/backend/db/__init__.py
...
**Files:**
- Create: `backend/app/schemas/fatigue.py`
...
Expected: `ImportError` or `ModuleNotFoundError` — `app.schemas.fatigue` does not exist yet.
...
```bash
git add backend/app/schemas/fatigue.py tests/backend/schemas/test_fatigue.py
...
Expected: `ImportError` — `app.schemas.athlete` does not exist yet.
...
```bash
git add backend/app/schemas/athlete.py tests/backend/schemas/test_athlete.py
...
Expected: `ImportError` — `app.schemas.plan` does not exist yet.
...
```bash
git add backend/app/schemas/plan.py tests/backend/schemas/test_plan.py
...
Expected: `ImportError` — `app.schemas.nutrition` does not exist yet.
...
```bash
git add backend/app/schemas/nutrition.py tests/backend/schemas/test_nutrition.py
...
Expected: `ImportError` — `app.schemas.review` does not exist yet.
...
```bash
git add backend/app/schemas/review.py tests/backend/schemas/test_review.py
...
Expected: `ImportError` — `app.db.database` does not exist yet.
...
```bash
git add backend/app/db/database.py tests/backend/db/test_models.py
...
Expected: `ImportError` or collection errors — `app.db.models` does not exist yet.
