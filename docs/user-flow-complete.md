...
> **Version 3 (finale A1).** Toutes les décisions prises en session A1 sont intégrées. Document stable, référence pour A2 (flow agent / orchestration LangGraph), A3 (roster des agents) et Phase B (spécification des schémas).
...
### État de données
`AthleteProfile` créé avec 4 champs : `date_of_birth`, `biological_sex`, `height`, `weight`. `coaching_scope` non défini. `assessment_mode` non défini.
...
### Entrée / sortie
Entrée : signup complété. Sortie : 4 champs valides → Phase 1.
...
### État de données
`coaching_scope` défini comme un dict par domaine fonctionnel.
...
- Si ≥ 1 domaine en `"full"` → Phase 2 (onboarding limité aux blocs pertinents).
...
### État de données
`ExperienceProfile`, `ObjectiveProfile`, `InjuryHistory`, `PracticalConstraints` enrichis. Classement 4 niveaux × 3 dimensions produit, accompagné d'un **niveau de confiance** par dimension.
...
- Pilotage par l'**Onboarding Coach**. Questions factuelles indirectes, jamais d'auto-évaluation subjective.
...
### Entrée / sortie
Entrée : `coaching_scope` avec ≥ 1 domaine en `"full"`. Sortie : blocs pertinents traités (remplis ou skippés) + synthèse validée → Phase 3.
...
### État de données
`assessment_mode = true`. `baseline_plan` généré à partir du **déclaratif onboarding uniquement**.
...
### Entrée / sortie
Entrée : onboarding validé. Sortie : confirmation baseline → Phase 4.
...
### État de données
`assessment_mode = true`. Logs session, HRV, sommeil, RPE, sensations, données de charge. `baseline_metrics` accumule.
...
- **Sécurité** : blessure rapportée ou douleur persistante → Recovery Coach prend la main.
- **Anomalie critique** : HRV en chute libre, RPE systématique ≥ 9 sur sessions sous-max → Head Coach surface.
...
### État de données
Transition `assessment_mode: true → false`. Premier plan personnalisé généré.
...
Head Coach en façade, Onboarding Coach alimente les questions en backend. Zones ciblées :
...
### Entrée / sortie
Entrée : baseline exécutée. Sortie : conditions + conversation validées → Phase 6.
...
### État de données
`assessment_mode = false`. `active_plan` créé.
...
### État de données
`active_plan` en cours. Logs continus. `AthleteState` mis à jour à chaque événement.
...
- Rapport de blessure → Recovery Coach prend la main.
- Changement d'objectif → Onboarding Coach, bloc objectifs.
- Changement de contraintes → Onboarding Coach, bloc contraintes.
...
**Maximum 2 messages Head Coach non sollicités par semaine**, hors rapport hebdomadaire.
...
- **Abandon / nouvel objectif principal** : Onboarding Coach complet sur bloc objectifs.
...
### Règles d'agents
- Onboarding Coach : pilote Phase 2 + conversation de suivi Phase 5 + ré-entrées (changement objectif / contraintes).
- Head Coach : façade constante, orchestre Phase 3 à 7, surface les anomalies.
- Recovery Coach : prend la main explicitement sur blessure + pause activité baseline.
- Coaches disciplines : travaillent en arrière-plan sauf exceptions cliniques.

---

## Suites

**A2 — Flow agent et orchestration LangGraph.** Traduit le flow utilisateur en graphe d'agents, transitions, conditions de routage, shared state, interruptions LangGraph. Entrées : ce document + charte de voix + principes déjà actés (consentement éclairé guidé, subordination, human-in-the-loop).

**A3 — Roster des agents consolidé.** Liste définitive des agents, responsabilités, scopes `_AGENT_VIEWS`, delta vs existant.

**Phase B.** Spécification exhaustive des sous-modèles de `AthleteProfile` et des ajustements `_AGENT_VIEWS`.

**Phase C.** Par agent, dans l'ordre : Onboarding → Lifting → Running → Nutrition → Recovery → Energy → Head Coach. C'est dans cette phase que sont formalisées les définitions opérationnelles par discipline (séance représentative, seuils, quantité minimale de signal).
